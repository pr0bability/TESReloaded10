...
float4 TESR_BillboardRight : register(c63);
float4 TESR_BillboardUp : register(c64);
float4 TESR_RockParams : register(c65);
float4 TESR_RustleParams : register(c66);
float4 TESR_WindMatrices[16] : register(c67);
float4 TESR_LeafBase[48] : register(c83);
...
#define	sqr(v)			((v) * (v))
	...
	if (TESR_ShadowData.x == 1.0f) { // Skinned (Actors)
		...
		q8.xyz = mul(float3x4(TESR_Bones[0 + offset.w].xyzw, TESR_Bones[1 + offset.w].xyzw, TESR_Bones[2 + offset.w].xyzw), q0.xyzw);
		q6.xyz = mul(float3x4(TESR_Bones[0 + offset.z].xyzw, TESR_Bones[1 + offset.z].xyzw, TESR_Bones[2 + offset.z].xyzw), q0.xyzw);
		q5.xyz = mul(float3x4(TESR_Bones[0 + offset.x].xyzw, TESR_Bones[1 + offset.x].xyzw, TESR_Bones[2 + offset.x].xyzw), q0.xyzw);
		q4.xyz = mul(float3x4(TESR_Bones[0 + offset.y].xyzw, TESR_Bones[1 + offset.y].xyzw, TESR_Bones[2 + offset.y].xyzw), q0.xyzw);
		...
		q9.xy = (((((q7.x * ((q7.x * q8.x) - 0.5)) + 1) * TESR_RustleParams.z) * TESR_RustleParams.x) / (PI * 2)) + const_4.yz;
		r1.xy = angler(frac(q9.xy)) * angler(frac(q9.xy));
		q3.xy = (((((q1.x * ((q1.x * q2.x) - 0.5)) + 1) * TESR_RockParams.z) * TESR_RockParams.x) / (PI * 2)) + const_4.yz;
		...
		r3.xw = (r1.yx * r3.yx) + 1;
		r3.y = -r3.w;
		r1.y = dot(r3.wxz, TESR_BillboardRight.xyz);
		r1.x = dot(r3.xyz, TESR_BillboardRight.xyz);
		q5.xy = (((((-2.52398507e-007 * q10.xy) + 2.47609005e-005) * q10.xy) - (1.0 / 72)) * q10.xy) + (1.0 / 24);
		r2.xy = (q5.xy * q10.xy) - 0.5;
		q10.xw = (q10.yx * r2.yx) + 1;
		r2.xyzw = IN.blendindexes.w * TESR_LeafBase[IN.blendindexes.z];
		q10.y = -q10.w;
		r4.x = dot(q10.zwx, r2.xyz);
		r5.x = dot(q10.zxy, r2.xyz);
		q10.y = dot(r3.wxz, TESR_BillboardUp.xyz);
		q10.x = dot(r3.xyz, TESR_BillboardUp.xyz);
		q10.zw = TESR_BillboardUp.zw;
		q59.xyzw = ((r4.x * q10.xyzw) + (r5.x * r1.xyzw)) + r0.xyzw;
		q28.xyzw = mul(float4x4(TESR_WindMatrices[0 + offset.x].xyzw, TESR_WindMatrices[1 + offset.x].xyzw, TESR_WindMatrices[2 + offset.x].xyzw, TESR_WindMatrices[3 + offset.x].xyzw), q59.xyzw);
